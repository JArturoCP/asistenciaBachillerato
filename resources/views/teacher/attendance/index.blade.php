<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            ...
            @if($selectedGroup)
                <a href="{{ route('teacher.attendance.export', ['group_id' => $selectedGroup->id, 'date' => $date]) }}" class="btn btn-outline-success btn-sm font-weight-bold">
    ...
    <div class="card card-custom p-4 bg-white mb-4">
        <form method="GET" action="{{ route('teacher.attendance.index') }}" class="row g-3 align-items-end">
            <div class="col-md-5">
                <label class="form-label fw-semibold text-dark">Grupo Asignado</label>
                <select name="group_id" class="form-select" onchange="this.form.submit()">
                    @forelse($groups as $group)
                        <option value="{{ $group->id }}" {{ $selectedGroupId == $group->id ? 'selected' : '' }}>
                            Grupo {{ $group->codigo_grupo }} - {{ $group->grado }}° Grado (Turno {{ ucfirst($group->turno) }})
                        </option>
                    @empty
                        <option value="">No tiene grupos asignados</option>
                    @endforelse
                </select>
            </div>

            <div class="col-md-4">
                <label class="form-label fw-semibold text-dark">Fecha de Consulta</label>
                <input type="date" name="date" class="form-control" value="{{ $date }}" onchange="this.form.submit()">
            </div>

            <div class="col-md-3">
                <button type="submit" class="btn btn-primary w-100 fw-semibold">
                    <i class="bi bi-search me-1"></i> Consultar Lista
                </button>
            </div>
        </form>
    </div>
    ...
    @else
        <div class="alert alert-warning card-custom text-center py-4">
            <i class="bi bi-exclamation-triangle fs-3 d-block mb-2"></i> No tiene grupos asignados en el ciclo escolar actual.
        </div>
    @endif
</x-app-layout>
