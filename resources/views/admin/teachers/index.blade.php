        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content card-custom">
                <form action="{{ route('admin.teachers.store') }}" method="POST">
                    ...
                    <div class="modal-body">
                        @if($errors->any() && old('form_modal_id') === 'modalCreateTeacher')
                            <div class="alert alert-danger alert-dismissible fade show small mb-3" role="alert">
                                <i class="bi bi-exclamation-triangle-fill me-1"></i> <strong>Verifique los siguientes errores:</strong>
                                <ul class="mb-0 mt-1 ps-3">
                                    @foreach($errors->all() as $error)
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                <label class="form-label fw-semibold">Nombre(s)</label>
                                <input type="text" name="nombre" class="form-control @error('nombre') is-invalid @enderror" value="{{ old('nombre') }}" placeholder="Ej. Roberto" required>
                            </div>
                            <div class="col-md-4 mb-3">
                                <label class="form-label fw-semibold">Apellido Paterno</label>
                                <input type="text" name="apellido_paterno" class="form-control @error('apellido_paterno') is-invalid @enderror" value="{{ old('apellido_paterno') }}" placeholder="Ej. Martínez" required>
                            </div>
                            <div class="col-md-4 mb-3">
                                <label class="form-label fw-semibold">Apellido Materno</label>
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Correo Electrónico (Login)</label>
                            <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}" placeholder="ejemplo@escuela.edu.mx" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Teléfono</label>
        ...
        <div class="modal-dialog">
            <div class="modal-content card-custom">
                <form action="{{ route('admin.teachers.storeMateria') }}" method="POST">
                    @csrf
                    <input type="hidden" name="form_modal_id" value="modalCreateMateria">
                    <div class="modal-header">
                        <h5 class="modal-title fw-bold"><i class="bi bi-book text-primary me-2"></i> Registrar Nueva Asignatura</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        @if($errors->any() && old('form_modal_id') === 'modalCreateMateria')
                            <div class="alert alert-danger alert-dismissible fade show small mb-3" role="alert">
                                <i class="bi bi-exclamation-triangle-fill me-1"></i> <strong>Verifique los siguientes errores:</strong>
                                <ul class="mb-0 mt-1 ps-3">
                                    @foreach($errors->all() as $error)
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="clave" class="form-control @error('clave') is-invalid @enderror" value="{{ old('clave') }}" placeholder="Ej. MAT-101" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Nombre de la Asignatura</label>
                            <input type="text" name="nombre" class="form-control @error('nombre') is-invalid @enderror" value="{{ old('nombre') }}" placeholder="Ej. Matemáticas I, Física II" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="semestre" class="form-select @error('semestre') is-invalid @enderror" required>
                                <option value="1" {{ old('semestre') == '1' ? 'selected' : '' }}>1er Semestre</option>
                                <option value="2" {{ old('semestre') == '2' ? 'selected' : '' }}>2do Semestre</option>
                                <option value="3" {{ old('semestre') == '3' ? 'selected' : '' }}>3er Semestre</option>
                                <option value="4" {{ old('semestre') == '4' ? 'selected' : '' }}>4to Semestre</option>
                                <option value="5" {{ old('semestre') == '5' ? 'selected' : '' }}>5to Semestre</option>
                                <option value="6" {{ old('semestre') == '6' ? 'selected' : '' }}>6to Semestre</option>
                            </select>
                        </div>
                    </div>
                    <div class="modal-footer">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="materia_id" class="form-select @error('materia_id') is-invalid @enderror" required>
                                ...
                                @foreach($materias as $materia)
                                    <option value="{{ $materia->id }}" {{ old('materia_id') == $materia->id ? 'selected' : '' }}>{{ $materia->nombre }} ({{ $materia->clave }} - {{ $materia->semestre }}° Semestre)</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-semibold">Hora Fin</label>
                                <input type="time" name="end_time" class="form-control @error('end_time') is-invalid @enderror" value="{{ old('end_time', '08:40') }}" required>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Aula / Salón (Opcional)</label>
                            <input type="text" name="aula" class="form-control @error('aula') is-invalid @enderror" value="{{ old('aula') }}" placeholder="Ej. Aula 102, Lab. Física">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancelar</button>
